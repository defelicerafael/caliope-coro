...
for($i=0; $i<count($dias); $i++){
    $array_desc = array("dia"=>$dias[$i]['numero_dia'],"mes"=>$dias[$i]['mes_texto'],"anio"=>$dias[$i]['anio'],"semana"=>$dias[$i]['numero_semana']);
    $descubrir = $consulta->getTableDataBusqueda('mes', $array_desc);
    if (count($descubrir) === 0) {
        $dias[$i]['menu_cargado'] = 'no';
    }else{
        $dias[$i]['menu_cargado'] = 'si'; 
    }
}
